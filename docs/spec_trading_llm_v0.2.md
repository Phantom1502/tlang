...
- `DIGIT`: 1 chữ số `0`–`9`, mỗi digit là 1 token riêng, **cách nhau bằng khoảng trắng** trong text thô (vd `<current_price> 0 5 4 3 </current_price>`). Zero-pad cố định **4 chữ số** cho phạm vi bin 0–1023 (`DIGIT_PAD=4`).
- `RR_TOKEN`: bracket-enum **atomic**, KHÔNG digit-decompose — chỉ 9 giá trị, tự mô tả rõ nghĩa hơn 1 con số trần. Không còn tiền tố `"RR:"` (khác `SL:` — SL vẫn cần nhãn vì là bin trần cần ngữ cảnh, RR không cần vì token đã tự chứa nghĩa).
- Ví dụ `action_block` đầy đủ: `<action> BUY SL: 0 4 9 5 <RR_9> </action>`.
- `BIN` (giá trị số nguyên của DIGIT{4} sau khi ghép, hoặc của candle atomic): 0–1023 (kế thừa từ `ChartCodec`).
...
**Nguyên tắc chốt cuối cùng: 2 chiến lược tokenize khác nhau cho 2 loại field, tùy vào việc model có cần LÀM PHÉP TÍNH SỐ HỌC trên field đó hay không.**

- **Chart OHLC — giữ ATOMIC** (`<O_543>`, `<H_543>`...): model chỉ cần *đọc* chart, không so sánh/tính toán trực tiếp trên OHLC → gộp tag+giá trị thành 1 token để compact (4 token/nến × 50 nến = 200 token/chart, thay vì 400+ nếu digit-decompose).
- **current_price / zone (2 cạnh) / SL — DIGIT-DECOMPOSE**: đây là các field model phải *so sánh, cộng trừ* (đúng phía zone, khoảng cách SL, current_price khớp Close...) → tách tag khỏi giá trị, mỗi chữ số là 1 token riêng, zero-pad cố định **4 chữ số** (`DIGIT_PAD=4`, phạm vi 0000–1023), các token digit cách nhau bằng khoảng trắng trong text thô. Vocab digit dùng chung chỉ 10 token (`0`-`9`) cho mọi field số này — cùng 1 không gian embedding, giúp model so sánh SL vs zone vs current_price dễ học hơn thay vì phải học lại quan hệ thứ tự riêng cho từng trong ~7000 embedding độc lập (nếu gộp tag+giá trị như OHLC).
  - Ví dụ: `<current_price> 0 5 4 3 </current_price>`, `<zone_support> 0 5 0 0 : 0 5 1 0 </zone_support>`, `SL: 0 4 9 5`.
  - Hệ quả phụ có lợi: vì chart giữ atomic còn `current_price` tách digit, rule "current_price phải khớp Close nến cuối" không còn là phép so khớp token-1-1 tầm thường — buộc model phải thực sự *giải mã* token chart rời rạc thành đúng chuỗi digit, đúng tinh thần auxiliary task ép model học map token↔bin.
- **RR — bracket-enum ATOMIC** (`<RR_1>`...`<RR_9>`): range quá nhỏ (1–9), không digit-decompose. Token tự mô tả rõ nghĩa, không cần tiền tố `"RR:"` (khác `SL:` — SL là bin trần cần nhãn ngữ cảnh, RR thì không).
- Token cấu trúc khác: mỗi tag/enum còn lại là 1 token riêng (`<chart>`, `<trend>UP</trend>`, `SL:`, `:`, `BUY`, `HOLD`...).
- Enum: `UP/DOWN/RANGE` (gộp trong tag `<trend>`), `BUY/SELL/CANCEL_BUY/CANCEL_SELL/WAIT_BUY/WAIT_SELL/HOLD` — mỗi giá trị 1 token.

**Vocab type khuyến nghị: WordLevel (exact-match cố định), KHÔNG dùng BPE.** BPE học merge theo tần suất thống kê — với 1 vocab đóng, hữu hạn, đã biết trước hết token hợp lệ, BPE có thể tự ý merge các digit hay đứng cạnh nhau (vd `"0"+"5"` → `"05"`), âm thầm phá vỡ đúng lợi ích digit-decompose (làm mất tính chất mỗi chữ số là 1 token độc lập). WordLevel + `WhitespaceSplit` pre-tokenizer đơn giản, không có chỗ để hành vi ẩn xảy ra — điều kiện bắt buộc: generator phải in token cách nhau đúng 1 khoảng trắng, kể cả từng digit.

**Nguồn sự thật duy nhất**: các hằng số `BIN_MIN=0`, `BIN_MAX=1023`, `DIGIT_PAD=4`, `RR_MIN=1`, `RR_MAX=9` được định nghĩa 1 lần ở `app/lang/tokens.py` — nhánh tokenizer nên import trực tiếp từ đây thay vì tự định nghĩa lại, tránh lệch giữa 2 nơi (bài học từ 1 bug thực tế: bản tokenizer đầu tiên của nhánh model tự ý "vá lành" cấu trúc hỏng trong `convert_tokens_to_string` — tự bịa giá trị/tag đóng khi thiếu digit hoặc thiếu tag, khiến lỗi well-form bị che giấu trước khi tới Parser. Nguyên tắc khắc phục: chỉ 1 tầng duy nhất (Parser, đã có panic-mode recovery) chịu trách nhiệm khoan dung lỗi cấu trúc — tokenizer encode/decode phải là phép ánh xạ trung thực, không tự sửa bất kể model sinh đúng hay sai).
