...
def make_chart(closes) -> str:
    ...
    Chart OHLC vẫn atomic <O_x> — không đổi so với trước."""
    candles = []
    for c in closes:
...
CASES = {
    ...
    "zone_direction_wrong_side": (
        ...
        + f" <think> <trend>UP</trend> <current_price> {fmt_bin(480)} </current_price> "
          f"<zone_support> {fmt_bin(500)} : {fmt_bin(510)} </zone_support> </think>"
    ...
    "price_in_zone_geometry_wrong": (
        ...
        + f" <think> <trend>UP</trend> <current_price> {fmt_bin(300)} </current_price> "
          f"<zone_support> {fmt_bin(500)} : {fmt_bin(510)} </zone_support> <price_in_zone> <good_price_action> </think>"
        + f" <action> BUY SL: {fmt_bin(290)} <RR_5> </action>"
    ),
